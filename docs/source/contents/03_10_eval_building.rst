...
.. math::
    ...
    \begin{align*}
        V_{leak,i,n} = \frac{ n_{leak,n} \cdot V_{rm,i} }{3600}
        \tag{2}
    \end{align*}

|   :math:`V_{leak,i,n}` : ステップ |n| における室 |i| のすきま風量, |m3| / s
|   :math:`n_{leak,n}` : ステップ |n| におけるすきま風による住宅全体の換気回数, 回 / h
|   :math:`V_{rm,i}` : 室 |i| の容積, |m3|
...
|   :math:`b_2` : 近似式の係数, 回 / h
|   :math:`C` : 相当隙間面積（C値）, ( |cm2| / |m2| )
|   :math:`\Delta \theta_n` : ステップ |n| における室内外温度差, K
...
ステップ |n| における室内外温度差 :math:`\Delta \theta_n` は次式で表される。
...
| :math:`\bar{\theta}_{r,n}` : ステップ |n| における平均室温, ℃ 
| :math:`\theta_{o,n}` : ステップ |n| における外気温, ℃ 

ステップ |n| における平均室温 :math:`\bar{\theta}_{r,n}` は次式で表される。

.. math::
    :nowrap:

    \begin{align*}
        \bar{\theta}_{r,n} = \dfrac{\sum\limits_{i} V_{rm,i} \theta_{r,i,n}}{\sum\limits_{i} V_{rm,i}}
        \tag{5}
    \end{align*}

| :math:`\theta_{r,i,n}` : ステップ |n| における室 |i| の温度, ℃ 
