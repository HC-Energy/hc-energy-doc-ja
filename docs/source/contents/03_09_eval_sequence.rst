...
ステップ |n| における境界 |j| の裏面温度 :math:`\theta_{rear,j,n}` で構成される :math:`J \times 1` のベクトル :math:`\pmb{\theta}_{rear,n}` は、次式により表される。
...
ステップ |n| からステップ |n+1| における室 |i| の人体発湿量 :math:`\hat{X}_{hum,i,n}` は次式により表される。

.. math::
    :nowrap:

    \begin{align*}
        \hat{X}_{hum,i,n} = \hat{X}_{hum,psn,i,n} \cdot \hat{n}_{hum,i,n} \tag{x}
    \end{align*}

ステップ |n| からステップ |n+1| における室 |i| の人体発熱量 :math:`\hat{q}_{hum,i,n}` は、次式により表される。

.. math::
    :nowrap:

    \begin{align*}
        \hat{q}_{hum,i,n} = \hat{q}_{hum,psn,i,n} \cdot \hat{n}_{hum,i,n} \tag{x}
    \end{align*}

ステップ |n| からステップ |n+1| における室 |i| の1人あたりの人体発湿量 :math:`\hat{X}_{hum,psn,i,n}` 及び、
ステップ |n| からステップ |n+1| における室 |i| の1人あたりの人体発熱量 :math:`\hat{q}_{hum,psn,i,n}` は、
...
室 |i| の微小球に対する境界 |j| の重み係数 :math:`f_{mrt,i,j}` は、
境界 |j| の面積 :math:`A_{s,j}` 、
...
境界に関する以下の値は、節「境界」で計算される値とする。

    1. 境界の数 :math:`n_b`
    2. 室 |i| と境界 |j| の関係を表す係数（境界jから室iへの変換） :math:`p_{i,j}`
    3. 境界 |j| の面積 :math:`A_{s,j}`, |m2|
    4. 境界 |j| の室内側表面放射熱伝達率 :math:`h_{s,r,j}`, W / ( |m2| K )
    5. 境界 |j| の室内側表面対流熱伝達率 :math:`h_{s,c,j}`, W / ( |m2| K )
    ...
    12. 境界 |j| の室内侵入日射の吸収の有無 :math:`B_{s,sol,abs,j}`
    13. 境界 |j| の熱損失係数（U値） :math:`U_j`, W / (|m2| K )
    14. ステップ |n| における境界 |j| の等価外気温度 :math:`\theta_{o,eqv,j,n}`, deg. C
    15. ステップ |n| における境界 |j| の侵入日射量 :math:`q_{trs,sol,j,n}`, W
    16. 境界 |j| の室内側長波長放射率 :math:`\varepsilon_{r,j}`, -
    17. 境界 |j| の項別公比法の指数項 |m| の公比 :math:`r_{j,m}`, -
    18. 境界 |j| の吸熱応答係数の初項 :math:`\phi_{a0,j}`, |m2| K / W
    19. 境界 |j| の項別公比法の指数項 |m| の吸熱応答係数 :math:`\phi_{a1,j,m}`, |m2| K / W
    20. 境界 |j| の貫流応答係数の初項 :math:`\phi_{t0,j}`, -
    21. 境界 |j| の項別公比法の指数項 |m| の貫流応答係数 :math:`\phi_{t1,j,m}`, -

^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
6. 室に関する値
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

室に関する以下の値は、節「室」で計算される値とする。
    ...
    10. 室 |i| の備品等の日射吸収割合 :math:`r_{sol,frt,i}`
...
ステップ |n| からステップ |n+1| における室 |i| のすきま風量 :math:`\hat{V}_{leak,i,n}` は、
ステップ |n| における室 |i| の温度 :math:`\theta_{r,i,n}` 、
ステップ |n| における外気温度 :math:`\theta_{o,n}`、及び
室 |i| の容積 :math:`V_{r,i}` に応じて、
節「建物全般」で計算される値とする。 
...
計算時間間隔 :math:`\Delta t` （単位:s）は、節「時間間隔」で計算される値とする。
