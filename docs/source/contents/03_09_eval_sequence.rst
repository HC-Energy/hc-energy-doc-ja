...
境界に関する以下の値は、節「境界」で計算される値とする。
    ...
    14. 境界 |j| の透過外気温度, ℃ :math:`\theta_{o,eqv,j,n}`
    15. 境界 |j| の侵入日射量, W :math:`q_{trs,sol,j,n}`
    16. 境界 |j| の室内側長波長放射率, -, :math:`\varepsilon_{r,j}`
    17. 境界 |j| の項別公比法の指数項 |m| の公比, -, :math:`r_{j,m}`
    18. 境界 |j| の吸熱応答係数の初項, |m2| K / W, :math:`\phi_{a0,j}`
    19. 境界 |j| の項別公比法の指数項 |m| の吸熱応答係数, |m2| K / W, :math:`\phi_{a1,j,m}`
